...
async function translatePo(poPath, loc, cfg, log = () => {}) {
  ...
  for (let i = 0; i < total; i += batchSize) {
    ...
    // Retry on failure — a malformed-JSON reply (unescaped quote in a
    // translation) is usually fine on the next attempt, so we don't lose a batch.
    ...
    for (let attempt = 1; attempt <= tries && !res; attempt++) {
      try {
        ...
        if (attempt === tries) log(`  ${loc.locale}: batch ${i} failed after ${tries} tries: ${err.message}`);
      }
    }
    if (!res) continue;
    chunk.forEach((it, j) => {
